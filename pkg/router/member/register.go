...
func Register(c echo.Context) error {
	...
	var userID int
		err = config.DB.QueryRow(query, member.Name, member.Username, string(hashedPassword)).Scan(&userID)
		if err != nil {if pqErr, ok := err.(*pq.Error); ok && pqErr.Code == "23505" { // 23505 is the PostgreSQL error code for unique violation
	...
	_, err = config.DB.Exec(query, member.Email, userID)
	if err != nil {
